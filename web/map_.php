          ...
          <!-- ========== Left Sidebar Start ========== -->
            ...
            <div class="content-page">
				...
				<?php
					...
					$sql ="SELECT weibo_id,substr(time,0,11) as time, category, location, event from earthquake_info where time between '2020-04-01 00:00' and '2020-04-13 00:00' group by location,substr(time,0,11)";
					...
					if(!$ret){
						...
						while($row = $ret->fetchArray(SQLITE3_ASSOC) ){
							...
							$sql ="SELECT * from earthquake_info where location='".strval($row['location'])."'and time between '2020-04-01 00:00' and '2020-04-20 00:00';";

							$ret3=$db->query($sql);
							$original_text=array();
							while($row3 = $ret3->fetchArray(SQLITE3_ASSOC)){
								$sql ="SELECT * from earthquake_originaltext where weibo_id='".strval($row3['weibo_id'])."'";
								$ret4=$db->query($sql);
			...
			for (i = 0; i < loc.length; i++) {
				(function(index){
					...
					marker[index].addEventListener("click", function (e) {
						...
						t.innerHTML="";
						t.innerHTML=' <div class="email-rightbar mb-3"><div class="card"><ul class="message-list">';
						for (var k=0;k<4 && k<weibo_text[index].length;k++){
