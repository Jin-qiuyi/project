          ...
          <!-- ========== Left Sidebar Start ========== -->
            ...
            <div class="content-page">
				...
				<?php
					...
					$sql ="select weibo_id,location,substr(time,0,11) as time,event from typhoon_info where weibo_id in (select weibo_id from typhoon_info where time between '2019-08-01 00:00' and '2019-10-06 00:00' group by location,substr(time,0,11) having count(weibo_id)>20)";
					...
					if(!$ret){
						...
						while($row = $ret->fetchArray(SQLITE3_ASSOC) ){
							...
							$sql ="SELECT * from typhoon_info where location='".strval($row['location'])."'and time between '2019-08-01 00:00' and '2019-10-06 00:00' limit 20;";

							$ret3=$db->query($sql);
							$original_text=array();
							while($row3 = $ret3->fetchArray(SQLITE3_ASSOC)){
								$sql ="SELECT * from typhoon_originaltext where weibo_id='".strval($row3['weibo_id'])."'";
								$ret4=$db->query($sql);
